...
- Contexto e KB do cliente nas pastas numeradas:
  - `00-contexto/` para resumo operacional, briefing, oferta, ICP e fontes.
  - `01-planejamento/` para metas, prioridades e sprint planning.
  - `02-dados/brutos/` e `02-dados/tratados/` para CSVs, JSONs, exports e planilhas processadas.
  - `03-campanhas/` para leituras de Meta Ads, Google Ads, TikTok Ads e diagnosticos por canal.
  - `04-copy/` para copys, angulos e criativos.
  - `05-experimentos/` para testes ativos e concluidos.
  - `06-relatorios/` para check-ins, funis, fechamentos semanais/mensais.
  - `08-decisoes/` para decisoes ja tomadas e proximos passos.
...
2. Leia o contexto nesta ordem:
   - `clientes/[cliente]/00-contexto/resumo-operacional.md`
   - `clientes/[cliente]/README.md`
   - arquivos recentes em `00-contexto/`, `01-planejamento/`, `06-relatorios/` e `08-decisoes/`
   - `clientes/[cliente]/CLAUDE.md`, se existir, apenas como apoio legado/instrucoes da IA.
3. Procure:
   - nicho
...
6. Se houver arquivos em `clientes/_inbox-revisar/`, nao use como fonte de cliente sem dono claro.
...
Para comparacao temporal, prefira janelas explicitas com `--since` e `--until` quando o periodo for conhecido:

```bash
python3 .claude/skills/v4mos-dados-meta-ads/scripts/v4mos_meta.py campaigns --cliente [cliente] --since AAAA-MM-DD --until AAAA-MM-DD --format json
```

Use `--days` apenas para leitura rapida. Se usar `--days 14` ou `--days 35`, confirme que o output permite separar buckets por data antes de calcular P0, P-1 ou MM4.
...
Se o cliente usar Kommo via V4mos, use a API quando houver `V4MOS_WORKSPACE_ID`, `V4MOS_CLIENT_ID` e `V4MOS_CLIENT_SECRET` no `.env` do cliente. Carregue o `.env` sem imprimir segredos:

```bash
set -a
source clientes/[cliente]/.env
set +a
curl -s "https://api.data.v4.marketing/v1/kommo/leads?workspaceId=$V4MOS_WORKSPACE_ID" \
  -H "x-client-id: $V4MOS_CLIENT_ID" \
  -H "x-client-secret: $V4MOS_CLIENT_SECRET"
```
...
Em vez de "chutar" o gargalo, rode a arvore por eliminacao. Siga a ordem, de cima pra baixo. Avance enquanto a etapa estiver saudavel. Pare na primeira falha com evidencia e marque essa falha como diagnostico principal; os demais pontos viram secundarios ou pendencias.
