...
Antes de criar, confira se ja existe uma skill com esse nome em `.claude/skills/{nome}/` OU em `.agents/skills/{nome}/`. Se existir:

- Pergunte: "Ja existe uma skill chamada `{nome}`. Voce quer (a) editar a existente, (b) escolher outro nome, ou (c) substituir?"
- Nunca sobrescreva sem confirmar.

### Passo 3 — Escrever o frontmatter completo

O frontmatter da skill V4 tem **5 campos obrigatorios**:
...
### Passo 4 — Escrever o corpo da skill

Siga os principios da secao avancada ("Skill Writing Guide" mais abaixo). Pontos importantes pro contexto V4:

- **Portugues brasileiro** em todo conteudo direcionado ao usuario final
- **Sem dados pessoais**: nao coloque nomes de clientes reais, credenciais, tokens, URLs internas privadas, nem exemplos com dados que pareceriam vazamento
- **Exemplos anonimizados**: "cliente X", "campanha Y", ou contextos fabricados
- Se a skill depende de MCP/API/ferramenta externa, documente o setup no topo do SKILL.md
- Idealmente inclua um **exemplo completo de uso** (input do usuario → comportamento da skill → output)
...
Apos criar a skill e confirmar que funciona pro usuario, sugira:

> "Skill criada e testada. Se voce quiser disponibilizar pro resto do time V4, roda `/compartilhar-skill` que eu empacoto, valido e abro um PR pro Builders Hub automatico."

Nao rode `/compartilhar-skill` automaticamente — deixe o usuario decidir quando compartilhar.

---

## Contexto do Skill-Creator original

Abaixo esta o conteudo original do criador-de-skills (evaluation, benchmarking, description optimization, packaging). Use quando:

- O usuario explicitamente pedir pra rodar evals quantitativos
- A skill sendo criada tem output objetivamente verificavel (transformacao de arquivo, extracao de dado, fluxo fixo)
- O usuario quer otimizar a descricao pra melhorar triggering

Pra skills simples do dia-a-dia V4 (copy, briefing, checkin, relatorio), o fluxo V4 acima e suficiente — pule direto pra criar e testar.

---

## Fluxo avancado (upstream criador-de-skills)
...
2. **Aggregate into benchmark** — run the aggregation script from the criador-de-skills directory:
   ```bash
...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ```bash
   nohup python <criador-de-skills-path>/eval-viewer/generate_review.py \
